function extractGeminiMessages() {
  ...
  const userEls = Array.from(document.querySelectorAll('user-query-content, user-query'))
  const aiEls = Array.from(document.querySelectorAll('model-response'))

  const allEls = [...userEls, ...aiEls].sort((a, b) => {
    const pos = a.compareDocumentPosition(b)
    return pos & Node.DOCUMENT_POSITION_FOLLOWING ? -1 : 1
  })

  allEls.forEach(el => {
    const isUser = el.tagName.toLowerCase().includes('user-query')
    let content = ''

    if (isUser) {
      const textEl = el.querySelector('p.query-text-line, .query-text, p')
      content = textEl?.textContent?.trim() || el.textContent?.trim() || ''
    } else {
      const textEl = el.querySelector('.response-content, div[class*="response-content"], .markdown')
      content = textEl?.textContent?.trim() || el.textContent?.trim() || ''
      content = content.replace(/^Show thinking\s*/i, '').trim()
    }
...
function showSummaryModal(text) {
  ...
  header.innerHTML = `
    <span style="font-weight: 700; font-size: 15px;">🧠 ContextSnap — Gemini</span>
...
function injectCapsuleButton() {
  ...
  // navigate to level6 = text-input-field (with-toolbox-drawer)
  const level6 = editor.parentElement?.parentElement?.parentElement?.parentElement?.parentElement?.parentElement
  if (!level6) return

  // leading-actions-wrapper is where + and Tools buttons live
  const leadingActions = level6.querySelector('.leading-actions-wrapper')
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'CAPTURE_CONTEXT') {
    ...
    if (messages.length === 0) {
      ...
      sendResponse({ success: true, messages, source: 'gemini' })
    }
  }
  return true
})