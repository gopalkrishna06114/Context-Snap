function extractPerplexityMessages() {
  ...
  const userEls = Array.from(document.querySelectorAll('h1[class*="group/query"], h1[class*="group"]'))
  const aiEls = Array.from(document.querySelectorAll('div.prose, div[class*="prose"]'))
...
function showSummaryModal(text) {
  ...
  header.innerHTML = `
    <span style="font-weight: 700; font-size: 15px;">🧠 ContextSnap — Perplexity</span>
...
function injectCapsuleButton() {
  ...
  const toolbar = document.querySelector('[contenteditable="true"][class*="overflow-auto"]')
  if (!toolbar) return
  ...
  // inject next to the input bar toggle indicator
  const toggleIndicator = document.querySelector('[data-testid="ask-input-mode-toggle-indicator"]')
  if (toggleIndicator && toggleIndicator.parentElement) {
    toggleIndicator.parentElement.appendChild(btn)
  } else {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'CAPTURE_CONTEXT') {
    ...
    if (messages.length === 0) {
      ...
      sendResponse({ success: true, messages, source: 'perplexity' })
    }
  }
  return true
})